...
- One AsyncEngine per tenant, kept in a registry dict.

- Each engine has a small pool (5 connections by default).

- For now all tenants point to the SAME database_url, but the registry

  pattern means we can give each tenant its own DB later without changing
...
# --- shared "system" engine for tenant lookup itself ---

# We need ONE engine to read the `tenants` table before we know which tenant

# we're serving. This is the bootstrap engine.

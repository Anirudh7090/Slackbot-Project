...
class Bot(Base, TimestampMixin):
    ...
    # Slug we use in logs, e.g. 'prj-sk' — appears in "[hello prj-sk]" responses
    slug: Mapped[str] = mapped_column(String(64), nullable=False)

    # Friendly display name
    name: Mapped[str] = mapped_column(String(128), nullable=False)

    # Slack credentials. In real production these should be encrypted at rest;
    # for local dev plain text is fine.
    ...
    # Slack's id for this bot's user account, e.g. 'U07ABCDEF'.
    # Slack includes this in events so we can identify the bot.
    slack_bot_user_id: Mapped[str] = mapped_column(
        String(32), index=True, nullable=False
    )

    # Slack app id, e.g. 'A07XYZ...'. Useful for debugging.

...
async def slack_events(
    ...
    # ---- Step A: URL verification handshake (Slack pings this once) ----
    if payload.get("type") == "url_verification":
        logger.info("Slack URL verification challenge received")
        return {"challenge": payload.get("challenge")}

    # ---- Step B: tenant + bot lookup ----
    tenant_id = getattr(request.state, "tenant_id", None)
    if not tenant_id:
        logger.warning("Slack event arrived with no tenant resolved")
        raise HTTPException(status_code=404, detail="tenant_not_found")

    # Open a tenant-scoped DB session manually (we're already past middleware)
    ...
    async with sm() as session:
        ...
        # ---- Step C: verify signature with THIS bot's signing secret ----
        ...
        # ---- Step D: dispatch the actual event ----
...
async def _handle_event(*, bot_id: int, tenant_slug: str, event: dict) -> None:
    ...
    async with sm() as session:
      ...
      # Map the Slack user
        ...
        # Get response from command handler (fixed commands)
